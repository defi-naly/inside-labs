...
const ExpandedCard = ({ data, onClose }: ExpandedCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-card shadow-2xl animate-in zoom-in-95 fade-in duration-300">
        <button
          onClick={onClose}
          className="absolute right-5 top-5 z-20 flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-card/80 backdrop-blur-sm transition-colors hover:border-primary/30"
        >
          ...
          <div className="p-8 md:p-12">
            ...
            <ul className="mb-8 space-y-2.5">
              {data.bullets.map((b, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm text-foreground">
                  <span
                    className="mt-1 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-primary"
                  >
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <a
                href="/omni-suite"
                onClick={(e) => e.stopPropagation()}
                className="inline-flex items-center gap-2 rounded-full border border-border px-5 py-2.5 text-sm font-semibold text-foreground transition-colors hover:border-primary/30"
              >
                See details
              </a>
            </div>
            ...
            <div className="mb-8 grid grid-cols-3 gap-4">
              {data.stats.map((stat, i) => (
                <div key={i} className="text-center">
                  ...
                  <div className="mt-1 text-xs text-secondary leading-tight">
                    {stat.label}
                  </div>
                </div>
              ))}
              ...
              <div className="flex items-end gap-1.5 h-20">
                {data.chartBars.map((height, i) => (
                  <div
                    ...
                    style={{
                      height: `${height}%`,
                      backgroundColor: i >= data.chartBars.length - 2
                        ? "hsl(355 78% 56%)"
                        : "hsl(355 78% 56% / 0.2)",
                    }}
                  />
